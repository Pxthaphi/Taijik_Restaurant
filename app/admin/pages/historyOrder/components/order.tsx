  ...
  const fetchOrders = async (status: string) => {
    try {
      ...
      let ordersQuery = supabase
        .from("orders")
        .select("Order_ID, Order_Datetime, Order_Status, Promotion_ID");
      ...
      const promotionIds = orders
        ...
        .filter((id) => id !== null); // Remove null Promotion_IDs
      ...
      const [
        ...
        promotionIds.length > 0
          ...
          : { data: [], error: null }, // Skip if no promotions
      ]);
      ...
      const enrichedOrders = orders.map((order: FetchedOrder) => {
        ...
        const finalPrice = totalPrice - promotionDiscount; // Apply discount
        ...
        >
          <div className="flex justify-between items-center mb-2 mt-2">
            ...
            <div className="text-green-500 text-xl font-DB_Med">
              ฿{order.finalPrice.toFixed(2)} {/* Use finalPrice here */}
            </div>
          </div>
        </Link>
      ))}
